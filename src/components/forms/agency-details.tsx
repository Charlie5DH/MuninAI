...
import { NewAgencyFormSchema as FormSchema } from "@/constants";

const AgencyDetails = ({ data }: { data?: Partial<Agency> }) => {
  const { toast } = useToast();
  const router = useRouter();
  const [deletingAgency, setDeletingAgency] = useState(false); // for the delete agency dialog
  ...
  const handleSubmit = async (values: z.infer<typeof FormSchema>) => {
    try {
      ...
      const response = await upsertAgency({
        ...
        // customerId: data?.customerId || custId || "", // stripe integration
      ...
      toast({
        title: "Created or modified Agency",
        duration: 5000,
      });
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                  ...
                  return (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border gap-4 p-4">
                      <div>
                        ...
                        <FormDescription>
                          Turning on Whitelabel mode will show your agency logo to all sub accounts by default. You can
              ...
              <Button
                type="submit"
                disabled={isLoading || !form.getValues().name || !form.getValues().companyEmail}
                className={`border ${
                  isLoading || !form.getValues().name || !form.getValues().companyEmail ? "bg-background" : "bg-primary"
                }`}
              >
